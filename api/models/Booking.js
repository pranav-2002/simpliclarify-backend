...
async function bookSlot(body) {
  try {
    ...
    if (!findMentor) {
      throw new CustomError({ statusCode: 404, message: "Mentor not found" });
    }
    ...
    if (findUser && !findUser.userVerify) {
      throw new CustomError({
        statusCode: 401,
        message: "Please verify your Email Address to proceed for booking",
      });
    }
    ...
    for (let i = 0; i < schedule.length; ++i) {
      if (
        !(await checkAvailability(mentorId, meetingDate, schedule[i].startTime))
      ) {
        isAvailableFlag = 1;
    ...
    if (promoId) {
      ...
      amount = Methods.applyDiscount(promo.promoDiscountPercentage, amount);
    }
    for (let j = 0; j < schedule.length; ++j) {
      if (!(await bookSchedule(mentorId, meetingDate, schedule[j].startTime))) {
        isBookedFlag = 1;
...
async function bookingSuccess(body) {
  ...
  try {
    ...
    if (generatedSignature !== razorpaySignature) {
      ...
      throw new CustomError("Booking verification failed, please try again");
    }
    ...
    if (paymentStatus === "CANCEL" || paymentStatus === "REJECT") {
      ...
      _capturePaymentCallback(capturePaymentObject, async function (response) {
        const obj = {
          razorpayPaymentId,
          razorpayOrderId,
          razorpaySignature,
        ...
        await Methods.bookingMailTemplate(user, mentor, foundBooking);
        const walletAmtDeducted = user.wallet - walletAmount;
        user.wallet = walletAmtDeducted;
        await user.save();
        if (walletAmount !== 0) {
          await addTransaction(user, walletAmount, _id, "Withdraw");
        }
      ...
      _capturePaymentCallback(capturePaymentObject, async function (response) {
        ...
        await Methods.bookingMailTemplate(user, mentor, foundBooking); // change
        const walletAmtDeducted = user.wallet - walletAmount;
        user.wallet = walletAmtDeducted;
        await user.save();
        if (walletAmount !== 0) {
...
async function _bookSlots(booking) {
  ...
  for (let j = 0; j < meetingTimings.length; ++j) {
    if (
      !(await bookSchedule(mentorId, meetingDate, meetingTimings[j].startTime))
    ) {
      booked = false;
      break;
    }
...
async function _cancelSchedule(mentorId, meetingDate, meetingTimings) {
  ...
  for (let i = 0; i < meetingTimings.length; i++) {
    if (
      await cancelSchedule(mentorId, meetingDate, meetingTimings[i].startTime)
    ) {
      continue;
    } else {
