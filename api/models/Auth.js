...
async function googleLogin(body) {
    try {
        ...
        if (!user) {
            ...
            await newUser.save()
            await addTransaction(newUser, Constants.joiningAmountWithoutReferral, 'Signup Reward')
            ...
            if (!user.isOAuth) {
                throw new CustomError({
                    statusCode: 403,
                    message: 'Account with same email id already exists! Try signing in directly / use forgot password',
                })
            }
...
async function signUp(body) {
    ...
    try {
        ...
        async function verifyReferralCode(referralCode, newUser) {
            ...
            if (adminReferrer) {
                if (adminReferrer.currentCount >= adminReferrer.limit || !adminReferrer.active) {
                    throw new CustomError({
                        statusCode: 401,
                        message: 'The Referral Code provided is no longer valid.',
...
async function login(body) {
    ...
    const user = await dbCallHandler(User.findOne({ userEmail }), 'User not found')
    if (user) {
        if (user.isOAuth) {
            throw new CustomError({
                statusCode: 403,
        ...
        const match = await Methods.comparePasswords(userPassword, user.userPassword)
        if (!match) {
            throw new CustomError({
                statusCode: 403,
...
async function verifyUser(body) {
    try {
        ...
        if (!user) {
            throw new CustomError('Looks like link has expired, please try again')
        }
        if (user.userVerify === true) {
            throw new CustomError('You are already verified. Have a good time!')
        }
        ...
        if (user.referralCodeUsed) {
            async function addAmountToWallet(user) {
                ...
                if (referrer) {
                    ...
                    await addTransaction(referrer, Constants.referrerAmount, 'Referral')
                    await referrer.save()
                    const subject = 'You just got RICHER!!'
                    const body = verifyUserEmailTemplate(Constants.referrerAmount)
                    ...
                    await addTransaction(user, Constants.referralAmount, 'Referral')
                    return
                }
            ...
            await addTransaction(user, Constants.joiningAmountWithoutReferral, 'Signup Reward')
        }
...
async function forgotPassword(body) {
    try {
        ...
        if (user.isOAuth) {
            throw new CustomError({
                statusCode: 403,
                message: 'User cannot reset password. Account logged in through google'
            })
        }
        const subject = 'SimpliClariFy Forgot Password'
        const generatedPassword = _generatePassword(8)
        const mailBody = forgotPasswordEmailTemplate(user.userName, generatedPassword)
        const mailSent = await sendMail(
            'hello@simpliclarify.com',
            user.userEmail,
            subject,
